...
class Server_Controller_Action_Helper_DecodeOauth2Token extends Zend_Controller_Action_Helper_Abstract {	
	...
	/**
	 * checks if the $certificate_path is absolute path then it returns it as it is,
	 * otherwise transforms it into bsolute path.
	 * @param string $certificate_path
	 * @return NULL|string
	 */
	private function fix_path($certificate_path) {
		if ($certificate_path == null)
			return null;
		if ($certificate_path{0} == DIRECTORY_SEPARATOR)
			return $certificate_path;
		
		return APPLICATION_PATH.DIRECTORY_SEPARATOR.'..'.DIRECTORY_SEPARATOR.$certificate_path;
	}
	
	
	/**
	 * checks the jws header
	...
	private static function is_valid_jws_header($header) {
		if (!is_array($header) || count($header) != self::$JWS_HEADER_COMPONENTS_COUNT)
			return false;
	...
	/**
	 * checks the jwe header
	 * @param array of strings $header
	 * @return boolean true if the $header is correct, false otherwise
	 */
	private static function is_valid_jwe_header($header) {
		if (!is_array($header) || count($header) != self::$JWE_HEADER_COMPONENTS_COUNT)
			return false;
		...
		/* the initialization vector must be a not empty string */;
	...
	/**
	 * checks the $jwt and if valid returns the data wrapped in Server_Model_Token
	 * @param array $jwt
	 * @return NULL|Server_Model_Token
	 */
	private static function get_token($jwt) {
		if (!is_array($jwt) || count($jwt) != self::$JWT_COMPONENTS_COUNT)
			return null;
	 ...
	 * @return number 1 if the $content_signature is the signature of $content with certificate $cert_location;
	...
	/**
	 * Takes in input a string encoding a JWE. If the 
	 ...
	 * 							  by Internet Draft http://tools.ietf.org/html/draft-ietf-jose-json-web-encryption-08
	 * @param Server_Model_ServerInfo $server_info the current resource server information
	 * @throws JWEException if there's something not conformant to the Internet Draft
	 * 						http://tools.ietf.org/html/draft-ietf-jose-json-web-encryption-08
	 * @throws JWSException if there's something not conformant to the Internet Draft
	 * 						http://tools.ietf.org/html/draft-ietf-jose-json-web-signature-08
	 * @throws JWTException if there's something not conformant to the Internet Draft
	 * 						http://tools.ietf.org/html/draft-ietf-oauth-json-web-token-06
	 * 						In this case the token has a parameter not defined by the current Internet Draft
	 * 						the parameter scopes
	...
	function direct($raw_token_data, $server_info) {
		...
		$signature_result = self::check_signature($jws[self::$JWS_HEADER_INDEX].
														self::$ENCODED_CHUNKS_SEPARATOR.
														$jws[self::$JWS_PAYLOAD_INDEX], 
												$jws_signature, 
												$this->fix_path($server_info->getPubKeyFile($token->get_issuer())));
